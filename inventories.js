...
const updateTableContent = (entity_id) => 
{
    ...
    document.getElementById('locationID').value = entity_id[1];
    document.getElementById('EquipmentID').value = entity_id[2];
    document.getElementById('quantity').value = entity_id[3];
    ...
    customerForm.addEventListener('submit', async function (e) 
    {
        ...
        const response = await fetch(`https://planetfitapi.azurewebsites.net/api/inventories/${selected_user}`, 
        {
            method: 'PUT',
...
const deleteTableContent = async (entity_id) => 
{
    ...
    if (confirm(`Are you sure you want delete ${entity_id[1]} from Inventory table ?`)) 
    {
        // Save it!
        const response = await fetch(`https://planetfitapi.azurewebsites.net/api/inventories/${entity_id[0]}`, 
        {
            method:'DELETE',
            headers: { 'Content-Type': 'application/json' }
            //body: JSON.stringify(Object.fromEntries(formData))
...
const postCustomer = () => 
{
    customerForm.addEventListener('submit', async function (e) 
    {
        ...
        const formData = new FormData(customerForm).entries()
        console.log(formData)
        const response = await fetch('https://planetfitapi.azurewebsites.net/api/inventories', 
        {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(Object.fromEntries(formData))
